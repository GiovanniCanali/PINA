import torch
import pytest
from pina.model.block import CoupledNetworkBlock
...
def test_constructor(inner_size):
    ...
    CoupledNetworkBlock(inner_size=inner_size, activation=torch.nn.Tanh)

    # Should fail if inner_size is negative
    with pytest.raises(AssertionError):
        CoupledNetworkBlock(inner_size=-1, activation=torch.nn.Tanh)


@pytest.mark.parametrize("inner_size", [10, 20])
def test_forward(inner_size):

    model = CoupledNetworkBlock(inner_size=inner_size, activation=torch.nn.Tanh)
